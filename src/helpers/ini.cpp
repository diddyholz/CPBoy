...
void *double_space(void *src, uint32_t *size, size_t elem_size)
{
  ...
  *size = (*size == 0)? 1 : *size * 2;
  ...
  void *new_space = malloc((*size) * elem_size);
...
{  
  ...
  {  
    switch (*p)
    {
      ...
      case '\n':
        // If reached end of line and tmp string is not empty and has key, add the value of the
        // tmp string as integer value to key. 
  ...
  uint32_t value_int, const char *value_str)
{
  ...
  if (!tmp_key) 
  {
    ...
    {
      section->keys = (ini_key *)double_space(section->keys, &(section->keys_size), sizeof(ini_key));
...
ini_section *add_section(ini_file *file, const char *name)
{
  ...
  if (!tmp_section) 
  {
    ...
    {
      file->sections = (ini_section *)double_space(file->sections, &(file->sections_size), sizeof(ini_section));
