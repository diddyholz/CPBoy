...
#define DIALOG_CONTROLS_HEIGHT      ALERT_CONTENT_OFFSET_Y + (DIALOG_CONTROLS_ITEM_COUNT * DEBUG_LINE_HEIGHT) + (2 * STD_CONTENT_OFFSET)
...
#define DIALOG_PALETTE_HEIGHT                 (ALERT_CONTENT_OFFSET_Y - DEBUG_LINE_HEIGHT) \
  + (3 * ((2 * STD_CONTENT_OFFSET) + DIALOG_PALETTE_COLOR_PANEL_HEIGHT + DEBUG_LINE_HEIGHT)) \
  + STD_CONTENT_OFFSET + DEBUG_LINE_HEIGHT
#define DIALOG_PALETTE_COLOR_RECT_HEIGHT      25
#define DIALOG_PALETTE_COLOR_PANEL_HEIGHT     (3 * (SLIDER_HANDLE_HEIGHT + DIALOG_PALETTE_SLIDER_SPACING)) \
...
void draw_controls_alert(emu_preferences *preferences, uint8_t selected_item)
{
  uint32_t position = draw_alert_box(
    ...
    TAB_SETTINGS_ITEM_CONTROLS_SUBTITLE,
  ...
  for (uint8_t i = 0; i < GB_KEY_COUNT; i++)
  {
    ...
    print_string_centered(
      title,
      dialog_x + (DIALOG_CONTROLS_WIDTH - (ALERT_CONTENT_OFFSET_X + (10 * (DEBUG_CHAR_WIDTH - 2)) + 2)),
      dialog_y + ALERT_CONTENT_OFFSET_Y + (i * DEBUG_LINE_HEIGHT),
      (DEBUG_CHAR_WIDTH - 2) * 10,
      0,
      COLOR_WHITE,
  ...
  print_string_centered(
    " OK ",
    dialog_x,
    dialog_y + ALERT_CONTENT_OFFSET_Y + (GB_KEY_COUNT * DEBUG_LINE_HEIGHT) + STD_CONTENT_OFFSET,
    DIALOG_CONTROLS_WIDTH,
    0,
    COLOR_WHITE,
    (selected_item == 8)? COLOR_SELECTED : COLOR_BLACK,
    true
  );
}
...
void draw_select_key_alert()
{
  uint32_t position = draw_alert_box(
    nullptr,
    nullptr,
    DIALOG_SELECT_KEY_WIDTH,
    DIALOG_SELECT_KEY_HEIGHT,
    COLOR_MENU_BG, 
    COLOR_PRIMARY
  );
...
void draw_edit_color_panel(uint16_t x, uint16_t y, uint16_t width, uint16_t *colors, 
  uint8_t selected_item, uint8_t selected_color)
{
  ...
  {
    draw_rectangle(
      x + (i * color_rect_width),
      y,
      color_rect_width,
      DIALOG_PALETTE_COLOR_RECT_HEIGHT,
      colors[i],
      (selected_color == i) * DIALOG_PALETTE_SELECTED_BORDER_WIDTH,
      (selected_item == 0)? DIALOG_PALETTE_SELECTED_COLOR : COLOR_WHITE
    );
  }

  const char *color_descriptions[] = {
    "Red",
    "Green",
    "Blue"
  };
  ...
  const uint16_t color_vals[] = {
    RGB565_TO_R(colors[selected_color]),
  ...
  {
    print_string(
      color_descriptions[i], 
      x, 
      y + DIALOG_PALETTE_COLOR_RECT_HEIGHT + STD_CONTENT_OFFSET + (i * (SLIDER_HANDLE_HEIGHT + DIALOG_PALETTE_SLIDER_SPACING)),
      0,
      COLOR_WHITE,
      COLOR_BLACK,
      true
    ...
    draw_slider(
      ...
      (selected_item == (i + 1)) ? COLOR_PRIMARY : COLOR_WHITE,
      0,
      color_max_vals[i],
      color_vals[i]
    );
...
void draw_edit_palette_alert(palette *pal, uint8_t selected_item, uint8_t *selected_colors)
{
  uint32_t position = draw_alert_box(
    pal->name,
    nullptr,
    DIALOG_PALETTE_WIDTH,
    DIALOG_PALETTE_HEIGHT,
    COLOR_MENU_BG, 
    COLOR_PRIMARY
  );
  ...
  const uint16_t dialog_x = ALERT_GET_X(position);
  const uint16_t dialog_y = ALERT_GET_Y(position);
  const uint16_t content_y = dialog_y + ALERT_CONTENT_OFFSET_Y - DEBUG_LINE_HEIGHT;
  ...
  for (uint8_t i = 0; i < 3; i++) 
  {
    print_string(
      titles[i],
      dialog_x + ALERT_CONTENT_OFFSET_X,
      content_y + (i * (DIALOG_PALETTE_COLOR_PANEL_HEIGHT + DEBUG_LINE_HEIGHT + (2 * STD_CONTENT_OFFSET))),
      0,
      COLOR_WHITE,
      COLOR_BLACK,
      true
    ...
    draw_edit_color_panel(
      dialog_x + ALERT_CONTENT_OFFSET_X,
      content_y + DEBUG_LINE_HEIGHT + (i * (DIALOG_PALETTE_COLOR_PANEL_HEIGHT + DEBUG_LINE_HEIGHT + (2 * STD_CONTENT_OFFSET))),
      DIALOG_PALETTE_WIDTH - 2 * (STD_CONTENT_OFFSET),
      pal->data[i],
      selected_item - (i * 4),
      selected_colors[i]
    );
  ...
  print_string_centered(
    " OK ",
    dialog_x,
    content_y + (3 * (DIALOG_PALETTE_COLOR_PANEL_HEIGHT + DEBUG_LINE_HEIGHT + (2 * STD_CONTENT_OFFSET))),
    DIALOG_PALETTE_WIDTH,
    0,
    COLOR_WHITE,
    (selected_item == 12)? COLOR_SELECTED : COLOR_BLACK,
    true
  );
}
...
{
  uint16_t dialog_height = ALERT_CONTENT_OFFSET_Y + ((user_palette_count + 2) * DEBUG_LINE_HEIGHT) + (2 * STD_CONTENT_OFFSET);

  uint32_t position = draw_alert_box(
    TAB_SETTINGS_ITEM_PALETTE_TITLE,
    TAB_SETTINGS_ITEM_PALETTE_SUBTITLE,
    DIALOG_PALETTE_WIDTH,
    dialog_height,
    COLOR_MENU_BG, 
    COLOR_PRIMARY
  ...
  print_string_centered(
    " Create New ",
    dialog_x,
    dialog_y + ALERT_CONTENT_OFFSET_Y + (user_palette_count * DEBUG_LINE_HEIGHT) + STD_CONTENT_OFFSET,
    DIALOG_PALETTE_WIDTH,
    0,
    COLOR_WHITE,
  ...
  print_string_centered(
    " OK ",
    dialog_x,
    dialog_y + ALERT_CONTENT_OFFSET_Y + ((user_palette_count + 1) * DEBUG_LINE_HEIGHT) + STD_CONTENT_OFFSET,
    DIALOG_PALETTE_WIDTH,
    0,
    COLOR_WHITE,
    (selected_item == (user_palette_count + 1))? COLOR_SELECTED : COLOR_BLACK,
    true
  );
}
...
int32_t edit_palette_alert(palette *pal) 
{
  ...
  uint8_t *selected_h_items[] = {
    &selected_colors[0], &colors[0][0], &colors[0][1], &colors[0][2], // OBJ0
    &selected_colors[1], &colors[1][0], &colors[1][1], &colors[1][2], // OBJ1
    &selected_colors[2], &colors[2][0], &colors[2][1], &colors[2][2], // OBJ2
    nullptr // OK
  };
  ...
  for (;;) 
  {
    ...
    if (memcmp(prev_selected_colors, selected_colors, sizeof(selected_colors)) != 0) 
    {
      // Get colors
      for (uint8_t i = 0; i < 3; i++) 
      {
    ...
    else
    {
      ...
      for (uint8_t i = 0; i < 3; i++) 
      {
        pal->data[i][selected_colors[i]] = RGB_TO_RGB565((uint16_t)colors[i][0], (uint16_t)colors[i][1], (uint16_t)colors[i][2]);
      }
    }
        ...
        if (selected_item == user_palette_count) 
        {
          ...
          if (ret == ERR_MAX_PALETTE_REACHED)
          {
            ok_alert(
              ...
              "You have already created the maximum of " XSTR(MAX_PALETTE_COUNT) " palettes!",
              COLOR_WHITE,
              COLOR_MENU_BG,
              COLOR_PRIMARY
            );
...
menu_tab *prepare_tab_settings(menu_tab *tab, emu_preferences *preferences)
{
  ...
  // Title for each item
  strcpy(tab->items[TAB_SETTINGS_ITEM_PALETTE_INDEX].title, TAB_SETTINGS_ITEM_PALETTE_TITLE);
  strcpy(tab->items[TAB_SETTINGS_ITEM_CONTROLS_INDEX].title, TAB_SETTINGS_ITEM_CONTROLS_TITLE);
  strcpy(tab->items[TAB_SETTINGS_ITEM_CREDITS_INDEX].title, TAB_SETTINGS_ITEM_CREDITS_TITLE);
