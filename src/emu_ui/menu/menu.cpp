...
void draw_pause_overlay()
{
  ...
  print_string_centered("Emulation paused", 0, 128, CAS_LCD_WIDTH, 0, COLOR_WHITE, COLOR_BLACK, 1);
  print_string_centered("Press [(-)] to continue", 0, 148, CAS_LCD_WIDTH, 0, COLOR_WHITE, COLOR_BLACK, 1);
}

void draw_menu_overlay()
{
  darken_screen_area(0, LCD_HEIGHT * 2, CAS_LCD_WIDTH, CAS_LCD_HEIGHT - (LCD_HEIGHT * 2));

  // print open menu text
  print_string_centered("Press [(-)] to open menu", 0, 402, CAS_LCD_WIDTH, 0, COLOR_WHITE, COLOR_BLACK, 1);
}

void draw_tab_bar(uint16_t x, uint16_t y, uint16_t width,
  uint8_t selected_tab, uint8_t tab_count, menu_tab *tabs) 
{
  ...
  for (uint8_t i = 0; i < tab_count; i++)
    ...
    const uint16_t tab_title_offset = (tab_width - (tab_title_len * (DEBUG_CHAR_WIDTH - 2))) / 2;

		print_string(
      tabs[i].title, 
      tab_offset + tab_title_offset,
			y + MENU_TAB_TITLE_OFFSET, 
      0, 
      COLOR_WHITE, 
      COLOR_BLACK, 
      true
...
void draw_tab_description(uint16_t x, uint16_t y, uint16_t width, menu_tab *tab)
{
...
{
  const uint16_t bottom_bar_pos = menu->y_pos + menu->height - MENU_TAB_HEIGHT - 1;
  ...
  draw_menu_items(
    menu->x_pos, 
    menu->y_pos + MENU_DESCRIPTION_HEIGHT + MENU_ITEM_Y_OFFSET, 
    menu->width, 
    menu->selected_item,
    &(menu->tabs[menu->selected_tab])
  );
...
uint8_t load_menu(emu_preferences *prefs)
{
  ...
  for (;;)
  {
    ...
    for (uint8_t i = 0; i < load_menu.tabs[load_menu.selected_tab].item_count; i++) 
    {
      h_items_count[i] = 1;
    }

    // Create horizontal items pointer array
    uint8_t *selected_h_items[load_menu.tabs[load_menu.selected_tab].item_count];
    ...
    // Process input
    // This will move the selected item and selected tab and check if the menu was closed
    // or an action should be executed
    return_code = process_input(
      selected_h_items, 
      &(load_menu.selected_item), 
      h_items_count, 
      load_menu.tabs[load_menu.selected_tab].item_count,
      load_menu.tabs[load_menu.selected_tab].items,
      true
    );
    ...
    if (return_code == INPUT_PROC_EXECUTE)
    {
      ...
      return_code = load_menu.tabs[load_menu.selected_tab]
        .items[load_menu.selected_item].action(
          &(load_menu.tabs[load_menu.selected_tab]
...
uint8_t emulation_menu(struct gb_s *gb, bool preview_only) 
{
  ...
  uint16_t *gb_frame_backup = (uint16_t *)hhk::malloc(LCD_HEIGHT * LCD_WIDTH * sizeof(uint16_t));
  ...
  if (gb_frame_backup)
  {
    for(uint16_t y = 0; y < LCD_HEIGHT; y++)
    {
      ...
      {
        gb_frame_backup[(y * LCD_WIDTH) + x] = vram[((y * 2) * CAS_LCD_WIDTH) + (x * 2)];
      }
    }
  }
  ...
  for (;;)
  {
    ...
    // Create horizontal items count array
    uint8_t h_items_count[emulation_menu.tabs[emulation_menu.selected_tab].item_count];
    
    for (uint8_t i = 0; i < emulation_menu.tabs[emulation_menu.selected_tab].item_count; i++) 
    {
      h_items_count[i] = TAB_COUNT;
    }

    // Create horizontal items pointer array
    uint8_t *selected_h_items[emulation_menu.tabs[emulation_menu.selected_tab].item_count];
    ...
    return_code = process_input(
      selected_h_items, 
      &(emulation_menu.selected_item), 
      h_items_count, 
      emulation_menu.tabs[emulation_menu.selected_tab].item_count,
      emulation_menu.tabs[emulation_menu.selected_tab].items,
      true
    );
    ...
    {
      // Execute action
      return_code = emulation_menu.tabs[emulation_menu.selected_tab]
  ...
  if (gb_frame_backup)
  {
    for(uint16_t y = 0; y < LCD_HEIGHT; y++)
    {
      for(uint16_t x = 0; x < LCD_WIDTH; x++)
      {
        vram[((y * 2) * (LCD_WIDTH * 2)) + (x * 2)] = gb_frame_backup[y * LCD_WIDTH + x];
        vram[((y * 2) * (LCD_WIDTH * 2)) + (x * 2) + 1] = gb_frame_backup[y * LCD_WIDTH + x];
        vram[(((y * 2) + 1) * (LCD_WIDTH * 2)) + (x * 2)] = gb_frame_backup[y * LCD_WIDTH + x];
        vram[(((y * 2) + 1) * (LCD_WIDTH * 2)) + (x * 2) + 1] = gb_frame_backup[y * LCD_WIDTH + x];
      }
    }
...
void draw_load_alert(void)
{
  ...
  uint32_t pos = draw_alert_box(nullptr, nullptr, LOAD_ALERT_WIDTH, LOAD_ALERT_HEIGHT, COLOR_MENU_BG, COLOR_PRIMARY);
  
  print_string_centered(
    "Loading ...", 
    ALERT_GET_X(pos), 
    ALERT_GET_Y(pos) + DEBUG_LINE_HEIGHT,
    LOAD_ALERT_WIDTH,
    0,
    COLOR_WHITE,
    COLOR_BLACK,
