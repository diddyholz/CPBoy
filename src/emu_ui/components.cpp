...
int string_split_newline(char *dest, size_t len, const char *src, uint16_t max_chars)
{
  ...
  uint16_t height, uint16_t background, uint16_t border)
{
  ...
  if (subtitle)
  {
    print_string_centered(
      subtitle, 
      x_pos, 
      y_pos + STD_CONTENT_OFFSET + DEBUG_LINE_HEIGHT,
      width,
      0,
      COLOR_DISABLED,
      COLOR_BLACK,
  ...
  uint16_t background, uint16_t border) 
{
  ...
  uint16_t width = text_len * (DEBUG_CHAR_WIDTH - 2) + (ALERT_CONTENT_OFFSET_X * 2);
  uint16_t height = TEXT_ALERT_MIN_HEIGTH + ((subtitle)? DEBUG_LINE_HEIGHT : 0);
  ...
  lines = string_split_newline(message, sizeof(message), text, TEXT_ALERT_MAX_CHAR);
  ...
  uint32_t position = draw_alert_box(title, subtitle, width, height, background, border);
  ...
  print_n_string(
    message,
    0,
    x + ALERT_CONTENT_OFFSET_X,
    y + ALERT_CONTENT_OFFSET_Y - ((subtitle)? 0 : DEBUG_LINE_HEIGHT),
    x + width - ALERT_CONTENT_OFFSET_X,
    0,
    foreground,
    background,
    true
  ...
  print_string_centered(
    " OK ",
    x + ALERT_CONTENT_OFFSET_X,
    y + ALERT_CONTENT_OFFSET_Y + ((lines - ((subtitle)? 0 : 1)) * DEBUG_LINE_HEIGHT) + STD_CONTENT_OFFSET,
    width - (2 * ALERT_CONTENT_OFFSET_X),
    0,
    foreground,
    COLOR_SELECTED,
    false
